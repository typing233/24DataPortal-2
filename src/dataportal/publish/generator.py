...
from dataportal.publish.render import (
    generate_render_yaml,
    generate_render_deploy_script,
    generate_render_logs_script,
    generate_render_rollback_script,
)
from dataportal.publish.cloudrun import (
    generate_cloudbuild_yaml,
    generate_deploy_script,
    generate_cloudrun_logs_script,
    generate_cloudrun_rollback_script,
)
...
class PublishGenerator:
    ...
    def generate(self) -> dict[str, str]:
        ...
        if self.platform in ("render", "all"):
            ...
            deploy_render = generate_render_deploy_script(port=self.port)
            dr_path = self.output_dir / "deploy-render.sh"
            dr_path.write_text(deploy_render)
            dr_path.chmod(0o755)
            generated["deploy-render.sh"] = str(dr_path)
...
"""
    ...
    def _generate_logs_script(self) -> str:
        if self.platform == "render":
            return generate_render_logs_script()
        elif self.platform == "cloudrun":
            return generate_cloudrun_logs_script()
        else:
            return """#!/bin/bash
...
echo "=== DataPortal Docker Logs (last $LINES lines) ==="
echo ""

docker compose logs --tail="$LINES" dataportal

echo ""
echo "--- Live tail: docker compose logs -f dataportal ---"
"""

    def _generate_rollback_script(self) -> str:
        if self.platform == "render":
            return generate_render_rollback_script()
        elif self.platform == "cloudrun":
            return generate_cloudrun_rollback_script()
        else:
            return """#!/bin/bash
...
if [ -n "$PREVIOUS_TAG" ]; then
    ...
    echo "Verify: docker compose logs -f dataportal"
else
